...
// Variables globales pour le pilotage (verrouillées une seule fois à l'extérieur pour éviter le stack overflow)
...
UNTIL runmode = 2 {
    ...
    IF runmode = 0 {
        ...
        // Assigner la commande de direction et de gaz (sans refaire de LOCK)
        SET steer_cmd TO HEADING(heading_cmd, pitch_cmd, 0).
        
        // Contrôle de la vitesse par la poussée (viser 75 m/s)
        LOCAL speed_error IS target_speed - SHIP:VELOCITY:SURFACE:MAG.
        SET throttle_cmd TO MIN(1.0, MAX(0.0, 0.4 + (speed_error / 20))).
    ...
    IF runmode = 1 {
        ...
        SET throttle_cmd TO 0.0.
        SET steer_cmd TO HEADING(runway_heading, 3, 0).
